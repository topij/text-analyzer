...
class KeywordAnalyzer(TextAnalyzer):
    ...
    def _extract_statistical_keywords(self, text: str) -> Dict[str, float]:
        ...
        if self.language_processor:
            ...
            base_forms = [
                self.language_processor.get_base_form(word)
                for word in words
            ]
        else:
    ...
    def _apply_predefined_keywords(
        self,
        ...
        for keyword, config in self.predefined_keywords.items():
            if self._keyword_in_text(keyword, text):
                keywords[keyword] = max(
                    keywords.get(keyword, 0),
                    config.importance
                )
    ...
    def _get_compound_words(
        self,
        text: str,
        keywords: List[tuple[str, float]],
        llm_compounds: List[str]
    ) -> List[str]:
    ...
    def validate_parameters(self, params: Dict[str, Any]) -> None:
        ...
        if "max_keywords" in params and (
            not isinstance(params["max_keywords"], int) or 
            params["max_keywords"] < 1
        ):
            raise ValueError("max_keywords must be a positive integer")
        
        if "min_keyword_length" in params and (
            not isinstance(params["min_keyword_length"], int) or
            params["min_keyword_length"] < 1
        ):
