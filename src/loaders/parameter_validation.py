...
class ParameterValidation:
    ...
    REQUIRED_FIELDS = {
        "general.max_keywords": int,
        "general.focus_on": str,
        "general.column_name_to_analyze": str,
    }
    ...
    FIELD_DEPENDENCIES = [
        {
            "fields": [
                "analysis_settings.weights.statistical",
                "analysis_settings.weights.llm",
            ],
    ...
    def _validate_patterns(
        ...
        for field_path, pattern in self.PATTERNS.items():
            try:
                if "*" in field_path:
                    self._validate_wildcard_pattern(
                        field_path, pattern, params, errors
                    )
                else:
                    value = self._get_nested_value(params, field_path)
                    if value is not None:
                        self._check_pattern_match(
                            value, pattern, field_path, errors
                        )
            except Exception as e:
                logger.error(
                    f"Pattern validation error for {field_path}: {str(e)}"
                )
        ...
        if isinstance(parent, dict):
            for key, obj in parent.items():
                try:
                    ...
                    if value is not None:
                        self._check_pattern_match(
                            value, pattern, child_path, errors
                        )
                except Exception as e:
                    logger.error(
                        f"Error in wildcard validation for {child_path}: {str(e)}"
                    )
    ...
    def _validate_keywords(
        self, params: Dict[str, Any], warnings: List[str]
    ) -> None:
        ...
        try:
            ...
            for category in categories.values():
                if isinstance(category, dict) and "keywords" in category:
                    if isinstance(category["keywords"], list):
                        ...
                        keywords = [
                            k.strip() for k in category["keywords"].split(",")
                        ]
            ...
            if conflicts:
                warnings.append(
                    f"Keywords appear in both predefined and excluded lists: {conflicts}"
                )
            ...
            for cat1 in categories:
                for cat2 in categories:
                    if cat1 < cat2:  # Compare each pair once
                        cat1_keywords = set(
                            categories[cat1].get("keywords", [])
                        )
                        cat2_keywords = set(
                            categories[cat2].get("keywords", [])
                        )
                        common = cat1_keywords & cat2_keywords
                        if common:
                            warnings.append(
                                f"Common keywords found in categories {cat1} and {cat2}: {common}"
                            )
        ...
        try:
            ...
            def check_cycle(category: str, path: Set[str]) -> bool:
                ...
                if parent:
                    if parent not in categories:
                        warnings.append(
                            f"Category {category} references non-existent parent {parent}"
                        )
                        return False
            ...
            for category in categories:
                if check_cycle(category, set()):
                    warnings.append(
                        f"Circular dependency detected in category hierarchy involving {category}"
                    )
    ...
    def _check_pattern_match(
        value: Any, pattern: str, field_path: str, errors: List[str]
    ) -> None:
        ...
        try:
            ...
            if not re.match(pattern, value_str):
                logger.debug(
                    f"Pattern match failed for {field_path}: value='{value_str}', pattern='{pattern}'"
                )
                errors.append(
                    f"Invalid format for {field_path}: must match pattern {pattern}"
                )
        except Exception as e:
    ...
    def _validate_ranges(
        ...
        try:
            for field_path, (min_val, max_val) in self.VALUE_RANGES.items():
                if "*" in field_path:
                    self._validate_wildcard_range(
                        field_path, min_val, max_val, params, errors, warnings
                    )
                else:
                    value = self._get_nested_value(params, field_path)
                    if value is not None:
    ...
    def _check_range(
        ...
        try:
            ...
            if num_value < min_val:
                errors.append(
                    f"Value for {field_path} ({num_value}) is below minimum {min_val}"
                )
            elif num_value > max_val:
                errors.append(
                    f"Value for {field_path} ({num_value}) is above maximum {max_val}"
                )

            # Add warnings for values close to limits
            elif num_value < min_val * 1.1:  # Within 10% of minimum
                warnings.append(
                    f"Value for {field_path} ({num_value}) is close to minimum {min_val}"
                )
            elif num_value > max_val * 0.9:  # Within 10% of maximum
                warnings.append(
                    f"Value for {field_path} ({num_value}) is close to maximum {max_val}"
                )

        except (TypeError, ValueError) as e:
            logger.error(f"Invalid numeric value for {field_path}: {str(e)}")
            errors.append(
                f"Invalid numeric value for {field_path}: must be a number between {min_val} and {max_val}"
            )
