...
class SemanticAnalyzer:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.text_processor = (
            create_text_processor(language)
    ...
    async def analyze(
        ...
        **kwargs
        ...
        processed_text = (
            ...
            if self.text_processor else text
        )
        ...
        if "keywords" in analysis_types:
            tasks.append(
                self.keyword_analyzer.analyze(
                    processed_text,
                    **kwargs.get("keyword_params", {})
                )
            )
        if "themes" in analysis_types:
            tasks.append(
                self.theme_analyzer.analyze(
                    processed_text,
                    **kwargs.get("theme_params", {})
                )
            )
        if "categories" in analysis_types:
            tasks.append(
                self.category_analyzer.analyze(
                    processed_text,
                    **kwargs.get("category_params", {})
                )
            )
    ...
    async def analyze_batch(
        ...
        **kwargs
        ...
        tasks = [
            self.analyze(text, analysis_types, **kwargs)
            for text in texts
        ]
