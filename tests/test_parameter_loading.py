...
from src.models.parameter_models import (
    ExtractionParameters, GeneralParameters,
    KeywordEntry, CategoryEntry
)
...
def create_test_excel(path: Path, language: str = "en") -> None:
    ...
    with pd.ExcelWriter(path, engine="openpyxl") as writer:
        ...
        pd.DataFrame({
            "keyword": ["education"],
            "reason": ["too general"]
        }).to_excel(writer, sheet_name="Excluded Keywords", index=False)
...
def test_parameter_validation():
    ...
    invalid_params = {
        "general": {
            "max_keywords": 25,  # Too high
            "language": "en",
            "min_keyword_length": 1  # Too low
        }
    }
...
def test_backward_compatibility():
        ...
        old_params = pd.DataFrame({
            'parameter': ['max_kws', 'focus_on', 'language'],
            'value': [5, 'test', 'en']
        })
