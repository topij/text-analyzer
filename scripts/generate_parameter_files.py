...
def get_parameter_data(language: str = "en") -> Dict[str, pd.DataFrame]:
    ...
    if language == "fi":
        ...
        general_params = pd.DataFrame(
            {
                ...
                "arvo": [
                    ...
                    "Finnish",
                    "Koulutuspalveluita tarjoavan yrityksen asiakaspalvelukeskustelut",
                    "keskustelu",
                ],
            }
        )
        ...
        excluded_keywords = pd.DataFrame({"avainsana": ["koulutus", "opiskelu"]})
        ...
        general_params = pd.DataFrame(
            {
                ...
                "value": [
                    ...
                    "English",
                    "Customer service conversations for an educational services company",
                    "text",
                ],
            }
        )
        ...
        excluded_keywords = pd.DataFrame({"keyword": ["education", "study"]})
...
def generate_parameter_files(output_dir: Path, language: str = "en") -> None:
    ...
    combined_params = pd.concat(
        [
            general_df,
            pd.DataFrame({param_key: ["categories"], value_key: [json.dumps(category_dict, ensure_ascii=False)]}),
            pd.DataFrame(
                {param_key: ["predefined_keywords"], value_key: [json.dumps(keyword_list, ensure_ascii=False)]}
            ),
            pd.DataFrame(
                {param_key: ["excluded_keywords"], value_key: [json.dumps(excluded_list, ensure_ascii=False)]}
            ),
        ]
    )
    ...
    combined_params.to_csv(
        csv_path, index=False, quoting=csv.QUOTE_ALL, encoding="utf-8-sig"  # For better Excel compatibility
    )
